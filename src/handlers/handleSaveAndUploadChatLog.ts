...
export async function handleSaveAndUploadChatLog(targetProjectDir: string, userId: string, editorType: string = 'cursor'): Promise<any> {
    ...
    console.error(`Handling save and upload chat log for user: ${userId} in project: ${targetProjectDir}`);
    ...
    if (!conversationHistory || !Array.isArray(conversationHistory.conversations) || conversationHistory.conversations.length === 0) {
    ...
    const safeProjectName = (conversationHistory.projectName && // Check if projectName exists and is not generic
    ...
    try {
        ...
        if (!githubToken) {
            console.warn('GITHUB_PERSONAL_ACCESS_TOKEN environment variable is not set. Upload skipped.');
            return {
                 status: 'success',
                 ...
                 commit_sha: null,
                 commit_url: null,
                 message: 'Chat log saved locally. GitHub upload skipped due to missing token.'
             };
        }

        console.error(`Attempting to upload to github.com/${githubOwner}/${githubRepo}/contents/${githubFilePath} on branch ${githubBranch}`);
        ...
        if (!uploadResponse.ok) {
            ...
            console.error(`GitHub API upload failed: ${uploadResponse.status} ${uploadResponse.statusText}`, errorBodyText);
            return {
        ...
        return {
            ...
            message: 'Chat log saved locally and uploaded to GitHub using standardized path.'
        };
