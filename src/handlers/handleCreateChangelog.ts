...
export async function handleCreateChangelog(
  ...
  try {
    const {
      ...
      pr_title,
      pr_body,
    } = validateArgs(CreateChangelogArgsSchema, args);
    ...
    console.error(
      `Handling create_changelog for: ${githubOwner}/${githubRepo}/${baseRepoPath}`,
    );
    ...
    const finalCommitMessage =
      commit_message || `docs: add changelog ${newFilename}`;
