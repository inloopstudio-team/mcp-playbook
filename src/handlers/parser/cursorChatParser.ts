...
interface ConversationHistory {
    ...
    conversations: { composerId: string; metadata?: ComposerMetadata; messages: ConversationMessage[] }[];
...
function safeJsonParse<T = any>(jsonString: string | null | undefined): T | null {
...
function inferProjectPathFromHistoryEntries(historyEntriesJson: string | null): string | null {
    ...
    for (let i = 1; i < paths.length; i++) {
        while (!paths[i].startsWith(commonPrefix) && commonPrefix.length > 0) {
            // Go up one level by finding the last directory separator
             ...
             if (lastSepIndex === -1) { // No separator found, possibly at root or invalid
                  commonPrefix = "";
                  break;
             }
...
function extractProjectNameFromPath(rootPath: string | null): string {
    ...
    if (isUserPath) {
        ...
        const knownProjects = ['genaisf', 'cursor-view', 'mcp-playbook', 'universal-github', 'inquiry'];
        ...
        if (!projectName && relativePathParts.includes('Documents') && relativePathParts.includes('codebase')) {
        ...
        const commonContainers = ['Documents', 'Projects', 'Code', 'workspace', 'repos', 'git', 'src', 'codebase', 'Downloads', 'Desktop', 'VCS'];
        if (projectName === username || (projectName && commonContainers.includes(projectName))) {
...
function extractProjectNameFromGitRepoPaths(gitRepoPathsJson: string | null): string | null {
             ...
             if (pathPartMatch && pathPartMatch[1]) {
                 ...
                 if (extractedPath) {
                     ...
                     const potentialName = path.basename(extractedPath);
                      ...
                      if (potentialName && !potentialName.match(/^[A-Z]:$/) && potentialName.toLowerCase() !== 'users') {
...
async function readWorkspaceDb(dbPath: string): Promise<WorkspaceDbData> {
    ...
    try {
        ...
        const aiServicePromptKeysSql = "SELECT key, value FROM ItemTable WHERE key LIKE 'aiService.prompts%'";
        const aiServiceGenKeysSql = "SELECT key, value FROM ItemTable WHERE key LIKE 'aiService.generations%'";
        ...
        for (const row of allRows) {
            ...
            if (key === 'history.entries') {
                ...
                chatData?.tabs?.forEach(tab => {
                    ...
                    if (!composerMetadata.has(composerId)) {
                         composerMetadata.set(composerId, { title: `Chat ${composerId.substring(0, 8)}` });
                    }
                });
                ...
                composerData?.allComposers?.forEach(comp => {
                    ...
                    meta.title = comp.name || meta.title || `Composer ${composerId.substring(0, 8)}`;
            ...
            } else if (key.startsWith('aiService.prompts') || key.startsWith('aiService.generations')) {
                 ...
                 aiData?.forEach(item => {
                      ...
                      if (composerId && text && typeof text === 'string') {
                           ...
                           messagesByComposer.set(composerId, currentMessages);
                            ...
                            if (!composerMetadata.has(composerId)) {
                                composerMetadata.set(composerId, { title: `AI Service ${composerId.substring(0, 8)}` });
                            }
                      }
                 });
            }
        ...
        console.error(`Error reading WORKSPACE database at ${dbPath}: ${error.message}`);
    } finally {
        if (db) {
            await closeDb(db);
        }
     ...
     const finalDebugSelectedRoot = parsedDebugSelectedRoot ? extractPathFromUri(parsedDebugSelectedRoot) : null;
     const finalGitRepoPaths = extractProjectNameFromGitRepoPaths(gitRepoPathsJson) ? [extractProjectNameFromGitRepoPaths(gitRepoPathsJson)!] : []; // Simplified to just return the first name found for now
...
async function readGlobalDb(dbPath: string): Promise<GlobalDbData> {
    ...
    try {
        ...
        const tableCheck = await getDbRow(db, "SELECT name FROM sqlite_master WHERE type='table' AND name='cursorDiskKV'");

        if (tableCheck) {
            // 1. Query bubbles from cursorDiskKV
            const bubbleRows = await getAllDbRows(db, "SELECT key, value FROM cursorDiskKV WHERE key LIKE 'bubbleId:%' ORDER BY rowid");
            for (const row of bubbleRows) {
                ...
                const bubbleData = safeJsonParse<{ text?: string; richText?: string; type?: number }>(valueJson);
                if (bubbleData) {
                     ...
                     // Add basic metadata if not present
                      if (!composerMetadata.has(composerId)) {
                          composerMetadata.set(composerId, { title: `Global Chat ${composerId.substring(0, 8)}` });
                     }
                }
            }

            // 2. Query composer data from cursorDiskKV
            const composerRows = await getAllDbRows(db, "SELECT key, value FROM cursorDiskKV WHERE key LIKE 'composerData:%'");
            for (const row of composerRows) {
                ...
                const composerData = safeJsonParse<{ name?: string; createdAt?: number; lastUpdatedAt?: number; conversation?: any[] }>(valueJson);
                if (composerData) {
                    // Update metadata
                    const meta = composerMetadata.get(composerId) ?? {};
                    meta.title = composerData.name || meta.title || `Global Composer ${composerId.substring(0, 8)}`;
        ...
        // 3. Query ItemTable in Global DB (less common but possible)
         const globalItemTableCheck = await getDbRow(db, "SELECT name FROM sqlite_master WHERE type='table' AND name='ItemTable'");
         if (globalItemTableCheck) {
              const globalChatDataRow = await getDbRow(db, "SELECT value FROM ItemTable WHERE key = 'workbench.panel.aichat.view.aichat.chatdata'");
              if (globalChatDataRow?.value) {
                   const chatData = safeJsonParse<{ tabs?: any[] }>(globalChatDataRow.value);
                   chatData?.tabs?.forEach(tab => {
                       const composerId = tab?.tabId;
                       if (!composerId) return;
                       const currentMessages = messagesByComposer.get(composerId) ?? [];
                       ...
                       // Add basic metadata if not already present
                       if (!composerMetadata.has(composerId)) {
                            composerMetadata.set(composerId, { title: `Global Tab ${composerId.substring(0, 8)}` });
                       }
                   });
              }
        ...
        console.error(`Error reading GLOBAL database at ${dbPath}: ${error.message}`);
    } finally {
        if (db) {
            await closeDb(db);
        }
...
async function findGlobalDbPath(userDataPath: string): Promise<string | null> {
         ...
         if (fsSync.existsSync(dirPath)) {
              try {
                   ...
                   for (const file of files) {
                        if (file.endsWith('.sqlite')) { // Prioritize .sqlite
    ...
    console.warn("Could not find a valid global Cursor database (state.vscdb, *.sqlite, *.db).");
    return null;
}
...
export async function getCursorConversationHistory(targetProjectDir: string): Promise<ConversationHistory | null> {
    console.error(`Attempting to parse Cursor chat history, target project directory: ${targetProjectDir}`); // Use log instead of error
    ...
    try {
        ...
        switch (platform) {
            case 'darwin': userDataPath = path.join(os.homedir(), 'Library', 'Application Support', 'Cursor', 'User'); break;
            case 'win32': userDataPath = path.join(process.env.APPDATA!, 'Cursor', 'User'); break;
            case 'linux': userDataPath = path.join(os.homedir(), '.config', 'Cursor', 'User'); break;
            default: console.error(`Unsupported OS: ${platform}`); return null;
        }
        ...
        if (workspaceDirs.length === 0) { console.error('No workspace directories found.'); return null; }
        ...
        for (const dirName of workspaceDirs) {
            ...
            } catch (error: any) { if (error.code !== 'ENOENT') console.warn(`Error stat-ing ${stateVscdbPath}: ${error.message}`); }
        }
        ...
        tempWorkspaceDbSqlitePath = path.join(os.tmpdir(), `cursor_workspace_${latestWorkspaceDirName}_${Date.now()}.sqlite`);
        ...
        if (globalDbPath) {
             tempGlobalDbSqlitePath = path.join(os.tmpdir(), `cursor_global_${Date.now()}.sqlite`);
        ...
        const globalData = tempGlobalDbSqlitePath ? await readGlobalDb(tempGlobalDbSqlitePath) : { messagesByComposer: new Map(), composerMetadata: new Map() };


        // 6. Merge Data from Workspace and Global DBs
        const allMessagesByComposer: MessagesByComposer = new Map(workspaceData.messagesByComposer);
        const allComposerMetadata: ComposerMetadataMap = new Map(workspaceData.composerMetadata);
        ...
        globalData.messagesByComposer.forEach((messages, composerId) => {
            ...
            const newMessages = messages.filter((m: ConversationMessage) => !existingContent.has(m.content));
            allMessagesByComposer.set(composerId, [...existingMessages, ...newMessages]);
        });
        ...
        globalData.composerMetadata.forEach((meta, composerId) => {
            if (!allComposerMetadata.has(composerId)) {
                allComposerMetadata.set(composerId, meta);
                 ...
                 existingMeta.createdAt = Math.min(existingMeta.createdAt || Infinity, meta.createdAt || Infinity);
                 existingMeta.lastUpdatedAt = Math.max(existingMeta.lastUpdatedAt || 0, meta.lastUpdatedAt || 0);
         ...
         try {
              ...
              } else if (Array.isArray(wsConfig?.folders) && wsConfig.folders.length > 0) {
         ...
         } catch (error: any) { if (error.code !== 'ENOENT') console.warn(`Could not read/parse workspace.json: ${error.message}`); }
         ...
         if (workspaceData.projectPaths.length > 0) {
              ...
              console.warn("Cannot set project path from Git data as only name is extracted currently.");
              // Let's try using targetProjectDir as a fallback if other methods fail
               if (fsSync.existsSync(targetProjectDir)) { // Basic check
          ...
          finalProjectPath = finalProjectPath ? path.normalize(finalProjectPath) : targetProjectDir; // Ensure we have *some* path
         ...
         if ((finalProjectName === 'Unknown Project' || finalProjectName === 'Home Directory') && workspaceName) {
              finalProjectName = workspaceName;
         }
          // If still generic, try the Git-derived name as a last resort
         if ((finalProjectName === 'Unknown Project' || finalProjectName === 'Home Directory') && workspaceData.gitRepoPaths.length > 0) {
         ...
         allMessagesByComposer.forEach((messages, composerId) => {
          ...
          conversations.sort((a, b) => (b.metadata?.lastUpdatedAt || 0) - (a.metadata?.lastUpdatedAt || 0));
        ...
        console.error('An unexpected error occurred during Cursor chat parsing:', error.message, error.stack);
        return null;
    } finally {
        // 8. Clean up temporary files
        if (tempWorkspaceDbSqlitePath && fsSync.existsSync(tempWorkspaceDbSqlitePath)) {
            try { await fs.unlink(tempWorkspaceDbSqlitePath); console.error(`Cleaned up ${tempWorkspaceDbSqlitePath}`); }
            catch (e: any) { console.error(`Error cleaning up ${tempWorkspaceDbSqlitePath}: ${e.message}`); }
        }
        if (tempGlobalDbSqlitePath && fsSync.existsSync(tempGlobalDbSqlitePath)) {
            try { await fs.unlink(tempGlobalDbSqlitePath); console.error(`Cleaned up ${tempGlobalDbSqlitePath}`); }
            catch (e: any) { console.error(`Error cleaning up ${tempGlobalDbSqlitePath}: ${e.message}`); }
        }
    }
}
...
export function formatConversationHistory(history: ConversationHistory): string {
    ...
    if (Array.isArray(history.conversations) && history.conversations.length > 0) {
        ...
        markdown += '_No relevant conversation history found for this workspace._\\n\\n';
    }
